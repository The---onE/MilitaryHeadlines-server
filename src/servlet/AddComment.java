...
public class AddComment extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (!Constants.isEmpty(articleId) && !Constants.isEmpty(userId) && !Constants.isEmpty(content)) {
				int aId = Integer.parseInt(articleId);
				int uId = Integer.parseInt(userId);
				int id = commentDao.addComment(aId, uId, content);
				if (id > 0) {
					res.put(Constants.RESPONSE_STATUS, Constants.STATUS_EXECUTE_SUCCESS);
					res.put(Constants.RESPONSE_PROMPT, "评论成功");
				} else {
					res.put(Constants.RESPONSE_STATUS, Constants.STATUS_ERROR);
					res.put(Constants.RESPONSE_PROMPT, "评论失败");
				}
			} else {
				res.put(Constants.RESPONSE_STATUS, Constants.STATUS_ERROR);
